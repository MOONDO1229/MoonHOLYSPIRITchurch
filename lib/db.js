...
export async function getSettings() {
  try {
    ...
    // 에러가 나거나 데이터가 없으면 기본 설정 반환
    if (error || !data || !data.data) {
      return DEFAULT_SETTINGS;
    }
    
    // 기본 설정과 저장된 설정을 병합하여 새로운 필드 누락 방지
    return {
      ...DEFAULT_SETTINGS,
      ...data.data
    };
  } catch (err) {
    console.error('getSettings fatal error:', err);
    return DEFAULT_SETTINGS;
  }
}
...
export async function getAlbumPosts() {
  ...
  const { data, error } = await supabase
    .from('album_posts')
    .select('*, album_images(id)')
    .eq('is_published', true)
    .order('event_date', { ascending: false, nullsFirst: false })
    .order('created_at', { ascending: false });

  if (error) {
    console.error('getAlbumPosts error:', error.message);
    return [];
  }
...
export async function getAlbumPostById(id, { includeUnpublished = false } = {}) {
  if (!supabase) return null;
  ...
  if (error) {
    console.error('getAlbumPostById error:', error.message);
    return null;
  }

  if (data?.album_images) {
    data.album_images = [...data.album_images].sort((a, b) => {
...
export async function getAllAlbumPosts() {
  if (!supabase) return [];
  ...
  if (error) {
    console.error('getAllAlbumPosts error:', error.message);
    return [];
  }

  return (data || []).map((post) => ({
    ...post,
    album_images: [...(post.album_images || [])].sort((a, b) => {
      const orderA = Number.isFinite(a.sort_order) ? a.sort_order : 0;
      const orderB = Number.isFinite(b.sort_order) ? b.sort_order : 0;
      return orderA - orderB;
    }),
